...
#Dicionary for spell. tells the spell its level and its abilty
		...
		'STINKING CLOUD':['concentration up to 1 min range 90ft, creates a 20ft sphere of yellow nauseating gas centered on a point in range. spreads around corners. each creature if in cloud makes a con save against poison if failed takes turn retching and reeling,' '3 rd level']
		}
        self.languages = {'READ': ['Halfling' ' Common'],
                         'WRITE': ['Halfing' ' Common']
            }
...
while exit != True:
    ...
    if command == "exit":
        print('Closing Loop')
        exit = True
    if command ==('Skills'): #if commanded skills then will print the skill function (skills with the modifire)
